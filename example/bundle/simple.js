...
var handler$1 = eventHandler({}); // private variables
...
var Modal = function Modal($el, id, options) {
  ...
  this.$el = $el;
  this.el = $el[0]; // assign the modal id

  this.id = id;
  this.opts = jquery.extend({}, _defaults, options);
  this.isOpen = false;

  if (this.opts.overlayHTML) {
    // create overlay
    this.$overlay = jquery(this.opts.overlayHTML); // append the modal and overlay to the body
...
Modal.prototype.close = function () {
  ...
  this.$el.attr(this.opts.attr, this.opts.offState); // restore the focus to the previously active element

  restore.focus();
  this.emit('close');
  handler$1.emit('close', this);
};
/**
...
var _setDefaults = function _setDefaults(defaults) {
  jquery.extend(_defaults, defaults);
};
/**
 * MODAL.create()
...
var _create = function _create(el, options) {
  ...
  var $el = jquery(el);
  if (!$el.length) return; // determine the modal id

  id = $el[0].id || '_' + ++_index; // return the modal if it exists
...
var modalHandler = jquery.extend(handler$1, {
  config: _config,
...
},{"../../cjs/modal-handler.js":1,"jquery":3}],3:[function(require,module,exports){
/*!
 * jQuery JavaScript Library v3.4.1
 * https://jquery.com/
 *
